...
// CAMBIO: Ahora subimos 3 niveles para llegar a src
...
const PlatilloItem = ({ producto }) => {
    ...
    return (
        ...
        >
            <span style={{
                fontWeight: 'bold',
                color: completado ? COLORES_RESTO.grisTexto : COLORES_RESTO.verde,
                marginRight: '12px',
                minWidth: '30px'
            }}>
                {producto.cantidad}x
            ...
            <span style={{
                fontSize: '0.9rem',
                color: completado ? COLORES_RESTO.grisTexto : 'white',
...
export const OrderCard = ({ pedido, onListo, onCancelar }) => {
    const [mostrarConfirmacion, setMostrarConfirmacion] = useState(false);

    return (
        <div style={{
            backgroundColor: COLORES_RESTO.tarjeta, borderRadius: '4px', border: `1px solid ${COLORES_RESTO.borde}`,
            ...
            {mostrarConfirmacion && (
                <div style={{
                    ...
                    backgroundColor: 'rgba(0,0,0,0.95)', zIndex: 10,
                    ...
                    <div style={{ display: 'flex', gap: '10px', width: '100%' }}>
                        <button onClick={() => { onCancelar(pedido._id); setMostrarConfirmacion(false); }} style={{ flex: 1, padding: '10px', backgroundColor: COLORES_RESTO.rojo, color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>SÍ</button>
            ...
            <div style={{ backgroundColor: COLORES_RESTO.cian, color: 'black', padding: '6px', textAlign: 'center', fontSize: '0.8rem', fontWeight: 'bold' }}>
                EN ESPERA
            </div>
            ...
            <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                <div style={{ display: 'flex', gap: '10px', marginBottom: '12px', borderBottom: `1px solid ${COLORES_RESTO.borde}`, paddingBottom: '10px' }}>
                    ...
                    <div style={{ fontSize: '0.75rem' }}>
                        <div style={{ color: 'white', fontWeight: 'bold' }}>Ticket: #{pedido._id.substring(pedido._id.length - 3)} / Mesa: {pedido.id_mesa?.substring(0, 2) || '69'}</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: 'auto', paddingTop: '10px' }}>
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <button style={{ flex: 1, padding: '10px', backgroundColor: COLORES_RESTO.verde, border: 'none', borderRadius: '4px', color: 'black', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.85rem' }}>
                            Tomar
                        </button>
                        <button
                            onClick={() => onListo(pedido._id)}
